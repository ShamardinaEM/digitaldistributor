...
export default function EmployeeSupportPage() {
    ...
    const { data: messages = [] } = useQuery({
        ...
        queryFn: async () => {
            ...
            const { data } = await apiClient.get<SupportMessage[]>(
                `/employee-support/requests/${selectedRequest}/messages`
            );
            return data;
        },
    ...
    const takeMutation = useMutation({
        mutationFn: async (requestId: number) => {
            await apiClient.patch(
                `/employee-support/requests/${requestId}/take`
            );
        },
        onSuccess: () => {
    ...
    const sendMessageMutation = useMutation({
        mutationFn: async ({
            ...
            await apiClient.post(
                `/employee-support/requests/${requestId}/messages`,
                { message }
            );
        },
        onSuccess: () => {
    ...
    const closeRequestMutation = useMutation({
        mutationFn: async (requestId: number) => {
            await apiClient.patch(
                `/employee-support/requests/${requestId}/close`
            );
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["support-requests"] });
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        {request.order.amount &&
                                            ` • ${request.order.amount.toLocaleString(
                                                "ru-RU"
                                            )} ₽`}
                                    </p>
                                </div>
